...
import {ToastConfirm} from "../components/ToastConfirm.tsx";
...
export const Dashboard = () => {
    ...
    const handleDelete = (e: React.MouseEvent, id: string) => {
        e.preventDefault();
        e.stopPropagation(); // Empêche d'ouvrir le détail du dossier
            ...
            <ToastConfirm
                ...
                onConfirm={async () => {
                    ...
                    try {
                        ...
                        // 3. Mise à jour de l'état local (UI) sans recharger la page
                        setRequests(prevRequests => prevRequests.filter(r => r.id !== id));

                        // 4. Succès (transforme le toast de chargement en succès)
                        toast.success("Brouillon supprimé !", { id: loadingToast });
                    } catch (err) {
                        // Si le backend dit "Impossible de supprimer un dossier soumis", l'utilisateur le verra.
                        toast.error(getErrorMessage(err, "Erreur lors de la suppression"));
                    }
                }}
            />
        ), {
            duration: 5000, // La question reste affichée 5 secondes max
            style: {
                background: '#fff',
                padding: '16px',
                borderRadius: '16px',
